...
const Dashboard = () => {
  ...
  const calculateTimeRemaining = () => {
    if (!chain) return { round: { days: 0, hours: 0 }, season: { days: 0 } };
    
    const start = new Date(chain.startDate);
    const now = new Date();
    const roundEnd = new Date(start);
    roundEnd.setDate(start.getDate() + (chain.currentRound * chain.roundDuration));
    ...
    return {
      round: {
        days: Math.floor(roundDiff / (1000 * 60 * 60 * 24)),
        hours: Math.floor((roundDiff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))
      },
      season: {
        days: Math.floor(seasonDiff / (1000 * 60 * 60 * 24))
      }
    };
  };
  ...
  const handleCompletePayment = () => {
    ...
    alert(`Payment of ${paymentAmount} ${chain?.currency} completed!`);
            ...
            <div className="mt-4 md:mt-0 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-indigo-50 p-4 rounded-xl">
                ...
                <p className="text-xl font-bold text-indigo-900">
                  {timeRemaining.round.days}d {timeRemaining.round.hours}h
                </p>
              </div>
              <div className="bg-purple-50 p-4 rounded-xl">
                <p className="text-sm text-purple-700 font-medium">Season Ends In</p>
                <p className="text-xl font-bold text-purple-900">
                  {timeRemaining.season.days} days
                </p>
              </div>
            </div>
          </div>
                          ...
                          >
                            ...
                            <div className="ml-4">
                              ...
                              <p className="text-sm text-gray-500">
                                {i === chain.currentRound ? `Ends in ${timeRemaining.round.days} days` : ''}
                              </p>
                            </div>
                          </div>
                        ))}
              ...
              <div>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                  ...
                  <button 
                    ...
                    onClick={() => document.getElementById('request-loan-modal')?.classList.remove('hidden')}
                  >
                    Request New Loan
                  </button>
                </div>
