...
// --- MODIFIED FORM SCHEMA FOR TWO TIERS ---
const formSchema = z.object({
  chainType: z.enum(['global', 'social']),
  chainFeature: z.enum(['standard', 'premium', 'enterprise']),
...
export function CreateChainPage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      chainType: 'global', // Default Chain Type
      chainFeature: 'standard', // Default Chain Feature
      name: '',
      description: '',
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const chainData: CreateChainFormData = {
        ...
        // Pass both new fields
        chainType: data.chainType,
        chainFeature: data.chainFeature
      }
  ...
  // --- MODIFIED INFO STRUCTURE ---
  const typeInfo = {
    global: {
      title: "Global Chain (DeFi Focus)",
      description: "Public chains emphasizing yield and liquidity. Backed by broader network collateral and governed by global platform rules.",
      icon: DollarSign,
    },
    social: {
      title: "Social Chain (Trust Focus)",
      description: "Private chains based on trust and community, ideal for groups with existing social collateral. Governed by group rules.",
      icon: Users,
    }
  }
  ...
  const featureInfo = {
    standard: {
      description: 'Basic features and standard support.',
      features: ['Basic automation', 'Standard support', 'Mobile app access'],
      fee: '0.5%', // Fee added here
    },
    premium: {
    ...
    enterprise: {
      ...
      fee: '1.0%', // Fee added here
    },
  }
  // --- END MODIFIED INFO STRUCTURE ---
              ...
              <CardContent>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    {/* --- START: 1. CHAIN TYPE SELECTION (FIRST FIELD) --- */}
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold">1. Chain Type (Focus)</h3>
                      
                      {/* CHAIN TYPE (Global/Social) */}
                      <FormField
                        control={form.control}
                        name="chainType"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Primary Chain Focus *</FormLabel>
                            ...
                            <FormDescription>
                              Choose the main goal: DeFi Yield (Global) or Community Trust (Social)
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    {/* --- END: 1. CHAIN TYPE SELECTION --- */}
                    
                    {/* Configuration Summary (Preview - Uses both type and feature) */}
                    {watchedValues.chainType && watchedValues.chainFeature && (
                      <motion.div
                        ...
                        <Card className="bg-muted/50">
                          <CardContent className="p-4">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <h4 className="font-semibold">{typeInfo[watchedValues.chainType].title} Chain ({watchedValues.chainFeature} Tier)</h4>
                                <p className="text-sm text-muted-foreground mb-2">
                                  {featureInfo[watchedValues.chainFeature].description}
                                </p>
                                <ul className="text-sm space-y-1">
                                  {featureInfo[watchedValues.chainFeature].features.map((feature, index) => (
                                  ...
                                  <li className="flex items-center gap-2 font-semibold text-primary">
                                    <div className="w-1.5 h-1.5 rounded-full bg-primary" />
                                    Platform Fee: {featureInfo[watchedValues.chainFeature].fee}
                                  </li>
                                </ul>
                              </div>
                            </div>
                    ...
                    {/* Financial Settings */}
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold">3. Financial Settings</h3>
                    ...
                    {/* Group Settings */}
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold">4. Group Settings</h3>
                    ...
                    {/* --- START: 5. CHAIN FEATURE SELECTION (LAST FIELD) --- */}
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold">5. Feature Tier (Pricing)</h3>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Select Feature Tier *</FormLabel>
                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                              <FormControl>
                                <SelectTrigger>
                              ...
                              <SelectContent>
                                {Object.entries(featureInfo).map(([feature, info]) => (
                            ...
                            <FormDescription>
                              The required platform fee is based on the selected feature tier.
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
